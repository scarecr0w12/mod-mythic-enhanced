...
-- Safe to apply even when the columns already exist.

ALTER TABLE `mythic_plus_level`
    ADD COLUMN IF NOT EXISTS `hp_mult` float NOT NULL DEFAULT 1.0 AFTER `random_affix_count`,
    ADD COLUMN IF NOT EXISTS `dmg_mult` float NOT NULL DEFAULT 1.0 AFTER `hp_mult`;
